...
```mermaid
flowchart TD
    A[局所の障害<br>中核設備の設定誤り]
    A -->|端末が位置登録を再送| B[正常な応答の集中<br>全国の交換設備が輻輳]
    B -->|61時間25分の継続| C[広域の途絶<br>音声 約2,316万人 データ 775万人以上]
    C -->|影響は機能の設計に依存| D[類型3の機能が停止<br>処理とデータが遠隔にあるもの]
    C -->|影響は機能の設計に依存| E[類型1と2は継続<br>端末側で完結 または一時途絶に耐える]
...
この図の前半は、検証報告書が記述した事実の経過です。後半は、途絶が生じた場合に影響を分ける要因の整理です。

前半で重要なのは、障害の原因が保守作業中の設定誤りという、特別ではない事象であった点です。攻撃ではなく、災害でもありません。日常的な作業の1つが、全国規模の途絶へ至りました。

後半で重要なのは、同じ途絶であっても、機能の設計によって影響が異なるという点です。類型1と類型2の機能は継続し、類型3の機能は停止します。
