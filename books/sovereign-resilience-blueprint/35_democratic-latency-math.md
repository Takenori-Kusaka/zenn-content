...
```mermaid
flowchart TD
    V[外部環境の変化速度<br>技術と情報環境では数年単位]
    D[決定に要する時間<br>争点認識 情報共有 討議 成文化 執行]
    V -->|速度が周期を上回る| G[差の拡大<br>決定時点で前提が変化している]
    D -->|事実認識の分裂と阻止権限の分散| G
    G -->|対応| R1[類型1. 周期の短縮<br>権限委任と期限設定]
    G -->|対応| R2[類型2. 対象の変更<br>事前の条件設定]
    G -->|対応| R3[類型3. 範囲の縮小<br>局所の裁量へ委ねる]
...
この図は、差の拡大が2つの方向から生じることを示しています。変化が速くなる方向と、決定が遅くなる方向です。

3つの対応類型は、それぞれ異なる代償を伴います。
