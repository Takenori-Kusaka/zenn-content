...
```mermaid
flowchart TD
    A[手続的な構成<br>更新と設定変更を逐次実行<br>現在の状態は履歴の累積]
    A -->|課題| B[把握の困難 復旧の困難<br>担当者の記憶への依存]
    C[宣言的な構成<br>望ましい状態を1つの記述として保持<br>記述から状態を生成]
    C -->|性質| D[記述が構成の完全な説明<br>別の機器に再現可能<br>世代として記録し戻せる]
    D -->|第2層以上で共有| E[拠点間で保守を融通<br>第Ⅴ部-6の制約への応答]
...
この図は、2つの構成方式を、課題への応答という観点で対比したものです。

本書が宣言的な構成を要件とする理由は、性能ではなく継続性にあります。第Ⅴ部-6で整理したとおり、局所の自律を制約する要因の1つは保守です。宣言的な記述は、保守に必要な知識を担当者から記述へ移し、拠点間で共有可能にします。

Eとして示した拠点間の融通は、第Ⅴ部-8の入れ子構造を情報基盤に適用したものです。1つの拠点で計算機の故障と保守者の不在が重なっても、記述を持つ他の拠点が復旧を支援できます。
