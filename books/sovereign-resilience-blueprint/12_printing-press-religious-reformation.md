...
宗教改革は、教義上の対立にとどまらず、政治的な対立と結合しました。1555年のアウクスブルクの和議は、領邦の統治者が自領の宗派を決定する原則を定めます。この対立は、第Ⅰ部A-1で記述した三十年戦争へと連なりました。
...
flowchart TD
    ...
    A --> B[帰結1. 同一性の確立<br>離れた読者が同じ本文を参照<br>議論の共通基盤が成立]
    A --> C[帰結2. 流通量の増大<br>短い小冊子が各地へ<br>教義上の主張が広範に到達]
    A --> D[帰結3. 言語の選択<br>ラテン語から各地の言語へ<br>読者層の拡大と分化]
    A --> E[帰結4. 権威の分散<br>解釈の主体が増加<br>教義の統一が困難に]
...
この図が示すのは、同一の技術変化が、方向の異なる4つの帰結を同時に生んだという点です。

帰結1と帰結4は、一見すると逆方向です。帰結1は共通の基盤を作り、帰結4は権威を分散させます。

しかし両者は矛盾しません。同一の本文を多数の人が参照できる状態は、その本文について各自が解釈することを可能にします。共通の基盤があるからこそ、解釈の差異が顕在化します。

帰結3も重要です。ラテン語による出版から各地の言語による出版へ移行すると、読者層が拡大する一方、言語圏ごとに読者が分かれます。

第Ⅰ部C-1で整理した4段階モデルに照らせば、この時期に生じたのは効果2から効果3への移行です。技術が組織を再設計し、それが職能の構成を変えました。写字生という職能は縮小し、印刷業者と組版工という職能が成立しました。
...
3.  **現代社会の現状と課題**
    ...
    生成の費用が低下した場合、この制約が緩みます。第Ⅲ部-5で述べるとおり、生成が安価で検証が高価であるとき、検証は現実的に実行されなくなります。

    したがって、印刷がもたらした帰結1、すなわち共通の本文を参照するという条件が、成立しにくくなる可能性があります。この論点は、第Ⅲ部で扱います。
