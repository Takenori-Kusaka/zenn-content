...
本章は、第Ⅴ部-3および第Ⅴ部-4で扱う設計原則の基礎です。また、第Ⅳ部-2で参照した研究の詳細にあたります。
...
```mermaid
flowchart TD
    P[共有資源の枯渇という問題<br>各利用者の合理的行動の総和が再生能力を超える]
    P --> A[選択肢1. 国家による管理<br>規則の制定と執行を中央が担う]
    P --> B[選択肢2. 私有化<br>資源を分割して所有権を付与]
    P --> C[選択肢3. 利用者による自主管理<br>8つの設計原則を満たす場合に持続<br>オストロムの観測]
...
この図が示すのは、選択肢3が観測に基づいて追加されたという点です。

選択肢1と選択肢2は、理論から演繹されました。選択肢3は、数百年にわたり持続した事例の観測から帰納されました。この方法上の差異が、オストロムの研究の性格を規定しています。

本書にとって重要なのは、選択肢3が条件つきであるという点です。8つの設計原則のいずれかが欠ければ、自主管理は持続しません。第Ⅰ部D-6で記述したアメリカ開拓期の鉱山規約が機能したのは、集団の規模が限定され、監視と制裁が実効的であった場合に限られていました。
...
3.  **現代社会の現状と課題**
    ...
    第Ⅱ部A-5で記述したとおり、集約型インフラの維持範囲は縮小に向かっています。その外側において、選択肢3が実際の選択肢となる区域が生じます。
