...
上段の問いは、左列と右列の2つの方向に分かれます。左列は作為による秩序であり、第Ⅰ部A-6で記述した中央集権化と、第Ⅱ部C-8で記述した統合管理の系譜にあたります。右列は作為の抑制であり、左列に対する対抗軸です。

右列の下の枠には、抑制の思想から導かれる2つの型が縦に並びます。上が無用の用、下が隠者の型です。

最下段の箱には、左列と右列の枠の両方から矢印が入ります。本書は右列を採用して左列を否定するものではありません。第Ⅱ部C-8で述べたとおり、統合管理は接続が維持されている間、効率と課題解決に寄与します。本書の設計は、左列が機能しなくなった期間に対するものです。
