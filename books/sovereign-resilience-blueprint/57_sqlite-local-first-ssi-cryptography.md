...
```mermaid
flowchart TD
    R[記録の保持<br>単一ファイルのデータベース<br>接続なしで読み書き 電源喪失に耐える]
    I[本人の同定<br>本人が管理する識別子と鍵<br>中央の認証機関に依存しない]
    I -->|記録への署名| S[署名つき記録<br>誰が作成したかを後から検証可能]
    R --> S
    S -->|複製を第2層以上へ| C[拠点間の照合<br>第Ⅴ部-3の原則4と原則6に対応]
...
この図は、記録と同定を組み合わせることで、共同管理の基礎が成立することを示しています。

記録だけでは、誰がその記録を作成したかを検証できません。同定だけでは、検証すべき記録がありません。両者を組み合わせた署名つき記録が、第Ⅴ部-4で述べた紛争解決の基礎となります。事実の認識の相違から生じる紛争は、署名つき記録の照合によって処理できます。

Cとして示した拠点間の照合は、第Ⅴ部-8の入れ子構造に対応します。記録の複製を第2層以上に置くことで、1つの拠点の記録が失われても復元でき、また複数の拠点の記録を照合して整合性を確認できます。
