---
title: "第Ⅴ部-16　情報の局所化Ⅱ ―― 記録の保持と、本人の同定"
---
...
同実装の公式文書は、この方式が適する用途と適さない用途を明示しています [ [ 249 ] ](https://sqlite.org/whentouse.html) 。多数の利用者が同時に書き込む用途には適さない一方、単一の機器または少数の利用者による記録には適するとされています。第Ⅴ部-8の第1層と第2層は、後者に該当します。
...
記録だけでは、誰がその記録を作成したかを検証できません。同定だけでは、検証すべき記録がありません。両者が合流した中段の署名つき記録が、第Ⅴ部-4で述べた紛争解決の基礎となります。事実の認識の相違から生じる紛争は、署名つき記録の照合によって処理できます。

下段の拠点間の照合は、第Ⅴ部-8の入れ子構造に対応します。記録の複製を第2層以上に置くことで、1つの拠点の記録が失われても復元でき、また複数の拠点の記録を照合して整合性を確認できます。

なお、この構成は第Ⅱ部C-9で記述した行政の番号制度を代替するものではありません。行政との連携が停止した期間において、局所の記録と同定を維持するための構成です。
---
