...
整備時期の集中が、どの経路で財政上の課題となるかを示します。

```mermaid
flowchart TD
    A[整備の集中<br>1960〜70年代に大量整備]
    A -->|耐用年数が同時に到来| B[更新期の集中<br>2040年に道路橋の約75％が50年超]
    B -->|管理主体は市町村が多数| C[負担の偏在<br>財政規模の小さい主体に集中]
    C -->|人口密度の低下| D[1人あたり負担の上昇<br>管路延長は減らず 利用者は減る]
    D -->|対応の選択| E[予防保全への転換<br>または供給範囲の見直し]
...
この図は、更新期の問題が単一の要因ではなく、3つの要因の重なりによって生じることを示しています。

第一の要因は時間の集中です。整備が集中したため、更新も集中します。これは施設の質とは無関係に、整備の時期から機械的に導かれます。

第二の要因は主体の偏在です。道路橋の65%を管理する市区町村は、財政規模が小さい傾向にあります。

第三の要因は人口密度の低下です。第Ⅱ部A-2で述べたとおり、管路や道路の延長は利用者が減っても短くなりません。

政府の対応方針である予防保全への転換は、第一の要因に対する応答です。費用の総額を抑制し、更新の時期を分散させます。

一方、第二と第三の要因については、費用の抑制だけでは解決しません。負担する主体の能力と、負担を分担する利用者の数が減少しているためです。この点で、供給範囲そのものの見直し、あるいは供給方式の転換が論点となります。
...
1.  **現代社会構造の原点**
    ...
    本章で示した施設群は、第Ⅰ部A-5で記述した全国一律の供給という設計思想の物理的な帰結です。人口と税収がともに拡大する局面において、全国に同じ水準の施設を整備することは、合理的な選択でした。
