...
```mermaid
flowchart TD
    Q[問い：使用量を計量できるか<br>支払わない者を排除できるか]

    Q -->|計量できる| A[方式1. 従量課金<br>電力 ガス 水道 通信<br>使用量と対価が対応]
    Q -->|計量できないが排除できる| B[方式2. 定額または会費<br>有料道路 会員制の施設<br>利用の可否のみを制御]
    Q -->|計量も排除もできない| C[方式3. 税による提供<br>街灯 治安 防疫 一般道路<br>使用量と負担が対応しない]
    Q -->|計量されない負荷| D[方式4. 価格に反映されない<br>大気 水域 生態系<br>過剰な負荷が生じる]
...
この図が示すのは、提供の方式が思想ではなく計量と排除の可否によって決まるという点です。

方式1が成立するのは、使用量が計測でき、かつ支払わない者を排除できる場合です。電力、ガス、水道、通信がこれに該当します。

方式2は、計量はできないが排除はできる場合です。

方式3は、計量と排除のいずれも成立しない場合です。街灯を1人だけ消すことはできません。したがって、費用は税として広く負担されます。

方式4が、本書の議論において重要です。ここに分類されるものは、価格を通じた調整の対象になりません。
