...
第Ⅲ部-4で整理したとおり、判断を委ねるか自ら行うかの区別は、環境の設計に依存します。本章は、判断の主体を利用者に残したまま、その負荷を下げる構成を、局所において実現する方法を扱います。
...
第三に、利用者がシステムの動作を変更できること。第Ⅲ部-4で述べた依存対象の移行を避けるための要件です。
...
左の経路の最下段の出力は、第Ⅲ部-4で述べた経路2、すなわち道具としての利用に対応します。判断の主体は利用者に残り、モデルは根拠つきの案を提示するに留まります。

右の破線の経路は、第Ⅴ部-8の方向2に対応する手動経路です。モデルが停止しても、記録への到達は失われません。
...
3.  **現代社会の現状と課題**
    ...
    第Ⅲ部-9で述べた悲観シナリオにおいて、供給の停止と情報環境の機能不全が重なる状況では、状況の把握に用いる情報の検証可能性が帰趨を分けます。本章の構成は、その検証可能性を局所で維持するためのものです。
