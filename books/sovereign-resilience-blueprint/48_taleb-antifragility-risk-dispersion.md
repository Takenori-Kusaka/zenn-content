...
```mermaid
flowchart TD
    V[変動の到来<br>障害 需要の急変 供給の途絶]
    V --> F[脆弱な構成<br>単一障害点を持つ<br>1点の停止が全体へ波及]
    V --> R[頑健な構成<br>冗長化により耐える<br>費用は増加 学習はしない]
    V --> A[反脆弱な構成<br>独立した局所単位の集合<br>一部の障害が他の改善に反映]
...
この図は、同じ変動が構成によって異なる帰結をもたらすことを示しています。

現在の集約型インフラは、効率を優先した結果として脆弱な構成へ近づいています。第Ⅱ部D-4で述べたとおり、これは設計の欠陥ではなく、稼働率を高める選択がもたらした帰結です。

頑健な構成への移行は、冗長化によって達成できます。ただし費用が増加し、第Ⅱ部A-4で示した更新期の負担をさらに重くします。

反脆弱な構成は、費用の構造が異なります。局所の単位は、それぞれが平時にも機能する設備であり、冗長化のためだけの設備ではありません。第Ⅲ部-7で述べた「備えを平時の機能に埋め込む」構成が、これにあたります。

ただしCとして示した成立条件が、この構成の実現を規定します。第Ⅴ部-3で整理した監視と参加の原則が、ここで必要になります。
