...
```mermaid
flowchart TD
    A[制約. 低電圧の直流では送電距離が限られる<br>供給範囲は発電所から約1km]
    A -->|変圧器と多相電動機の実用化| B[選択. 交流方式の採用<br>昇圧と降圧により長距離送電が成立]
    B -->|発電所を需要地から離せる| C[帰結1. 発電の大規模化<br>水力立地の利用 設備の集約]
    C -->|設備費用は稼働の有無によらず発生| D[帰結2. 利用率の最大化<br>需要時間帯の異なる顧客を同一系統へ]
    D -->|配電網の重複を避ける必要| E[帰結3. 規制つき地域独占<br>供給義務と料金規制を伴う]
...
この図が示すのは、現在の電力事業の形が、送電の物理的な性質から導かれているという点です。

出発点は、抵抗による損失という物理的な制約です。この制約が、交流方式の採用を導きました。

交流方式の採用は、発電所の立地を需要地から解放しました。これにより、発電は大規模化し、水力など特定の立地条件を要する電源が利用可能になります。

帰結2は、設備の費用構造から導かれます。発電設備の費用は、その大部分が初期投資です。稼働率が低ければ、単位あたりの原価は上昇します。したがって、可能な限り多くの需要を同一の設備に接続することが、原価の低減につながります。

帰結3は、配電網の重複を避けるという要請から導かれます。同一の道路に2社が電線を敷設することは、設備費用の二重負担を生みます。この構造は、自然独占と呼ばれる状態にあたります。

この連鎖の全体が、物理的な制約から出発している点が重要です。事業者の意図や当時の思想が起点にあるのではありません。

同時に、逆の含意もあります。物理的な制約が変われば、この連鎖の帰結も変わりうるということです。第Ⅴ部では、この点を検討します。
