...
```mermaid
flowchart TD
    A[段階1. 広域インフラの整備<br>道路 水道 防御施設<br>継続的な維持費用を伴う]
    A -->|3世紀の危機 通貨の劣化と侵入| B[段階2. 行政と軍の拡大<br>統治の分担 属州の細分化<br>維持費用のさらなる増加]
    B -->|耕作地の減少と実効支配の縮小| C[段階3. 徴税基盤の縮小<br>北アフリカなど主要供給地の喪失]
    C -->|支出削減の対象化| D[段階4. インフラ維持の断念<br>石造建築の停止 陶器の地域化<br>貨幣流通の縮小]
    D -->|広域の分業と交換の縮小| E[段階5. 統治単位の縮小<br>所領単位の防衛と裁判<br>荘園の形成]
...
この図が示すのは、インフラの崩壊が破壊によってではなく、維持の断念によって進行したという構造です。

段階2において注意すべき点があります。危機への対応として実施された再編は、行政機構と軍の規模を拡大させました。これは危機対応として合理的な選択でしたが、同時に維持費用を増加させました。

第Ⅰ部A-6で参照するテインターの枠組みは、ここにも当てはまります。問題への対応として複雑性を追加すると、その複雑性そのものが維持費用となります。追加された複雑性の利得が維持費用を下回るようになると、系は余力を失います。

段階4で観測された変化は、本書にとって重要です。石造建築の停止、陶器の地域化、貨幣流通の縮小。これらはいずれも、広域の分業と交換が縮小したことを示します。

第Ⅰ部B-2で述べるとおり、分業の程度は市場の規模に制約されます。逆に言えば、広域の交換が縮小すると、分業も縮小します。分業が縮小すると、生産される財の質と種類が低下します。

段階5は、この条件下における適応です。広域の供給が期待できない状況で、生存に必要な機能を局所で完結させる構成が広がりました。

なお、この段階5を単なる後退とみなす評価には無理があります。広域の交換が成立しない環境において、局所完結型の構成は合理的な適応です。ただし、以前の生活水準まで保たれたわけではありません。
