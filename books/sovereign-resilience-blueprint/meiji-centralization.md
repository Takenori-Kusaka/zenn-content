---
title: "第Ⅰ部A-5　明治国家の中央集権化 ―― 一元的な統治OSはどう設計されたか"
---
...
```mermaid
flowchart TD
    R[1871年 廃藩置県<br>藩という統治単位の法的消滅]

    R --> F1[機能1. 人事の集約<br>府県長官を中央が任命]
    R --> F2[機能2. 財政の集約<br>地租改正：地価基準・金納]
    R --> F3[機能3. 武力の集約<br>徴兵令・秩禄処分]
    R --> F4[機能4. 人材規格の集約<br>学制による統一課程]
...
この図が示しているのは、中央集権化が単一の法令によってではなく、4つの独立した機能の集約によって達成されたという構造です。

廃藩置県は、藩という単位を法的に消滅させました。しかし、それだけで統治が成立するわけではありません。誰が地方を運営するのか（人事）、その運営費用をどこから得るのか（財政）、秩序を維持する実力をどこが保持するのか（武力）、そして機構を動かす人員をどう供給するのか（人材）。この4つが、それぞれ別個の制度によって中央へ移されました。

4つの制度は、いずれも1871年から1876年という短期間に集中しています。この同時性は偶然ではありません。1つでも欠けた場合、旧来の権力基盤が残存し、集約が完了しないためです。

なお、統治の正統性を成文化する憲法は、これら実務的な集約の完了から13年後に発布されました。実力の集約が先行し、正統性の成文化が後続したという順序は、この転換の性格を示しています。
...
3.  **現代社会の現状と課題**
    ...
    第Ⅱ部では、これらの機能が現在どの程度有効に働いているかを、公的統計に基づいて検討します。第Ⅲ部では、いずれかの機能が失効した場合に何が起こるかを、シナリオとして扱います。
