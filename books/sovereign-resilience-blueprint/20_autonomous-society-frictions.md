...
判断の負荷という観点から、移行の条件を整理します。

```mermaid
flowchart TD
    Q[現状：集約型への依存が既定]
    Q -->|判断の負荷が低い| A[選択1. 現状維持<br>仕組みの理解も対処も不要<br>費用は金額へ圧縮される]
    Q -->|判断の負荷が高い| B[選択2. 局所自律への移行<br>方式の選択 設備の導入<br>異常時の対処が必要]
    B -->|負荷の非対称を縮小する設計| C[条件. 平時は意識されず<br>異常時は自動で切り替わり<br>状態は開示され変更もできる]
...
この図が示すのは、移行の可否が意識や理念だけでは決まらず、負荷の配分に強く依存するという点です。

選択1と選択2の間にある負荷の差が、移行を妨げます。この差を縮小しない限り、選択2は選ばれにくい状態が続きます。

条件として示した3項目は、いずれも設計上の要件です。
