...
```mermaid
flowchart TD
    L1[層1. 実証された事実<br>評価課題における性能の推移<br>公表結果として確認可能]
    L1 --> L2[層2. 前提つきの推定<br>計算資源と算法改善の想定に基づく時系列<br>前提が変われば結果も変わる]
    L2 --> L3[層3. 構想と方針<br>汎用AIの実現と統治の仕組み<br>意思の表明であり事実ではない]
    L3 --> G[手続の空白<br>拘束力のある規制は成立しつつある<br>個別判断への開示と不服申立は未整備]
...
この図が示すのは、開発主体が公表する文書を読む際に、その確度を区別する必要があるという点です。

層1の事実を根拠に層3の構想を確実なものとして扱うことは、推論として成立しません。逆に、層3が構想にすぎないことを理由に、層1の事実を軽視することも適切ではありません。

Gが本書にとって重要です。第Ⅲ部-5で整理する依存対象の移行は、この手続の空白の中で進行します。行政手続には決定の根拠を開示し不服を申し立てる制度がありますが、開発主体の判断について同等の手続は保証されていません。EUのAI法や日本のAI推進法は、この空白を縮小する方向の制度ですが、個別の判断に対する手続を直ちに保証するものではありません。
