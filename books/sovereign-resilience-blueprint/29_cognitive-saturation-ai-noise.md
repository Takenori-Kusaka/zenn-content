...
```mermaid
flowchart TD
    G[生成費用の低下<br>文章 画像 音声の大量生成]
    G -->|区別が困難な情報の増加| V[検証費用の相対的な上昇<br>個々の情報に照合が必要]
    V -->|検証が実行されない| L[検証の省略<br>情報の真偽が判定されないまま流通]
    G -->|生成物が学習データに混入| M[出力の多様性の喪失<br>2024年の実証研究]
    L -->|手段の設計に依存| R[検証を可能にする環境<br>出典への到達 発信者の同定 独立情報源の照合]

    style G fill:#1e293b,stroke:#94a3b8,color:#f1f5f9
    style V fill:#292524,stroke:#a8a29e,color:#fafaf9
...
この図が示すのは、問題が情報の量そのものではなく、生成と検証の費用比にあるという点です。

情報の量は、第Ⅰ部C-4で記述した活版印刷以来、増加を続けてきました。量の増加だけであれば、新しい現象ではありません。

新しいのは、生成の費用が検証の費用を大きく下回る状態です。この状態では、Lとして示した検証の省略が、個々の受け手にとって合理的な選択になります。

Mとして示した観測は、この構造が生成AI自身に及ぶことを示します。生成物が学習に混入すると、出力の多様性が失われます。これは情報環境の劣化が、自己強化的に進行しうることを意味します。

Rとして示した環境は、第Ⅴ部で扱う設計要件に対応します。検証を受け手の努力に委ねるのではなく、検証に要する費用を環境の側で下げる構成です。
