...
```mermaid
flowchart TD
    A[設計：過去の観測統計<br>治水 排水 送電の外力想定]
    A -->|過去の分布が続くと仮定| B[定常性の仮定<br>一定確率の外力の<br>大きさは変わらない]
    O[観測：分布の変化<br>50mm/h以上の大雨<br>約226回→約334回 1.5倍]
    B --> C[ずれ：想定を超える頻度<br>施設の欠陥ではない]
    O --> C
    C -->|新しい分布に合わせる| D[選択1. 基準の引き上げ<br>費用と工期が増加]
    C -->|超過を前提とする| E[選択2. 超過時の運用設計<br>避難 縮退運転 復旧手順]

    style A fill:#1e293b,stroke:#94a3b8,color:#f1f5f9
    style B fill:#292524,stroke:#a8a29e,color:#fafaf9
    style O fill:#1e293b,stroke:#94a3b8,color:#f1f5f9
...
3.  **現代社会の現状と課題**
    ...
    第Ⅲ部-3で定義する区分Aのうち、気候の変化は階層2に属します。すなわち、傾向が持続する場合に外挿できる事象です。
