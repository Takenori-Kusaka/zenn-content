...
```mermaid
flowchart TD
    P5[原則5. 段階的な制裁<br>警告 制限 除外<br>初期段階は自動化可能]
    P5 --> P6[原則6. 紛争解決の場<br>記録の照合で解決できる部分と<br>合議を要する部分の区別]
    P6 --> P7[原則7. 組織化の承認<br>法制度の問題<br>規制の目的を維持した形態の承認]
    P7 --> P8[原則8. 入れ子状の組織<br>局所は単独で完結せず<br>上位と接続され融通を受ける]

    style P5 fill:#0f3d2e,stroke:#10b981,color:#ecfdf5
    style P6 fill:#292524,stroke:#a8a29e,color:#fafaf9
    style P7 fill:#3f1d1d,stroke:#dc2626,stroke-width:2px,color:#fef2f2
    style P8 fill:#1e293b,stroke:#94a3b8,stroke-width:2px,color:#f1f5f9
```

#### 図の読み方

この図は、後半の4原則を実装の性質によって配置したものです。

原則5と原則6は、前章の原則4、すなわち監視の記録に依存します。記録が正確で参照可能であれば、制裁の初期段階と紛争の多くは、記録によって処理できます。

原則7は、性質が異なります。これは共同管理の側ではなく、外部の制度の側の問題です。技術がいかに整っても、制度が形態を認めなければ成立しません。第Ⅰ部A-4で記述した江戸期の幕藩体制は、分権的な統治が上位の承認のもとで長期に持続した事例として参照できます。

原則8は、第Ⅳ部-2で述べた地域社会への期待の階層と対応します。局所の単位は、災害直後の相互援助から供給の共同維持へ機能を拡張する際、単独では完結しません。上位との接続が、持続の条件です。
