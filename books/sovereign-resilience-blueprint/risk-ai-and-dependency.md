---
title: "第Ⅲ部-4　生成AIがもたらす3つの変化 ―― 労働・情報環境・依存対象の移行"
---
...
この論点に関する知見は、第Ⅲ部-5で詳述します。そこで参照するのは、フロムによる自由と不安の分析、カーネマンによる判断の2つの系の整理、サミュエルソンとゼックハウザーによる現状維持バイアスの実験、そしてフィスクとテイラーによる認知的な倹約家の概念です。
...
```mermaid
flowchart TD
    T[生成AIが専門領域で<br>実用水準に到達]
    T --> Q[問い：人は何を選ぶか<br>決めるのは負荷の差]
    Q -->|判断の負荷が低い| P1[経路1. 判断の委託<br>依存先が事業者へ移動<br>不服申立の手続なし]
    Q -->|負荷を引き受ける| P2[経路2. 道具として利用<br>判断の主体は個人<br>検証の手間を負担]
    P2 -->|成立に必要な設計| D[負荷を下げる設計<br>根拠の提示 情報源へ到達<br>動作を変更できる]
...
第Ⅲ部-5で整理する知見に照らせば、左の経路1の選ばれやすい条件が揃っています。判断を委ねることは負荷が低く、現状を変えないことは負荷が低く、検証を省くことは負荷が低いためです。
...
1.  **事実と推論の境界**
    ...
    本章において事実として記述したのは、次のものです。生成AIの導入効果に関する観測結果。情報の生成費用と検証費用の非対称。そして、第Ⅲ部-5で詳述する社会心理学および行動経済学の既存の知見です。
