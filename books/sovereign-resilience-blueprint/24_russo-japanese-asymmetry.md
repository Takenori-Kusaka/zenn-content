...
[第Ⅰ部D-1](war-why-states-fight)で整理した枠組みに照らすと、この開戦には、将来の力関係に関する予測が作用していました。ロシアはシベリア鉄道の完成によって極東への輸送能力を高めつつあり、日本側には時間の経過が不利に働くという認識がありました。
...
この戦争において機能した方式が、その後の環境の変化にもかかわらず維持されたことは、[第Ⅰ部A-8](dynastic-cycle-and-corruption)で整理した枠組みにおける硬直化の事例として位置づけられます。

### 🗺️ 図：非対称な条件下での勝利と、その後の硬直化

国力で劣る側の勝利がどの条件で成立し、その後に何を残したかを示します。

```mermaid
flowchart TD
    A[条件. 国力で劣る側<br>相手の輸送能力が<br>完成する前に時期を選択]
    A -->|情報 同盟 目的の限定| B[戦術的勝利の連続<br>旅順 奉天 日本海]
    B -->|継続能力の限界| C[帰結1. 講和<br>目的の一部を達成<br>賠償金は獲得できず]
    B -->|成功した方式の基準化| D[帰結2. 硬直化<br>艦隊決戦と白兵突撃を<br>環境変化後も基本に]
    D --> E[次の環境で不適合<br>条件の検証を伴わない継承]
...
この図は、上の2段を1本の鎖として読み、2段目の箱から左右に分かれる形をしています。同じ勝利が2つの異なる帰結を持ったことを示します。

最上段の箱と矢印の脇の語が、2段目の勝利の条件です。[第Ⅰ部D-1](war-why-states-fight)で整理した枠組みと対応し、将来の力関係に関する予測が開戦時期を決め、情報と目的の限定が戦術的な優位を生みました。

左の帰結1は、[第Ⅰ部D-5](war-blitzkrieg-doctrine)で整理する短期決着の論点と対応します。戦術的な勝利が継続能力の限界と結びついたとき、講和の内容は勝利の程度に見合わないものとなりました。

右の帰結2が本書にとって重要です。特定の条件下で機能した方式が、条件が変わった後も維持されました。右の列だけ最下段まで続けたのは、この硬直化が次の環境まで影響を残したためです。

第Ⅰ部D-5で述べるとおり、ある戦争様式の有効性はその成立条件に依存します。条件の検証を伴わない継承は、次の環境において機能しません。
...
3.  **現代社会の現状と課題**
    ...
    第二の論点は、[第Ⅲ部](future-method)の議論に接続します。現在の供給インフラと意思決定制度は、20世紀の条件下で成功した方式を継承しています。[第Ⅱ部](japan-current-state)で確認するとおり、その条件の一部は変化しています。

    条件の検証を伴わない継承が硬直化を招くという本章の論点は、[第Ⅲ部-10](scenario-comparison)で整理する早期警戒指標の背景にあります。
