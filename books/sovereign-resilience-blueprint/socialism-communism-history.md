...
flowchart TD
    ...
    A -->|方法をめぐる分岐| B1[経路1. 議会を通じた漸進改革<br>再分配と社会保障<br>生産手段は私有のまま]
    A -->|方法をめぐる分岐| B2[経路2. 権力掌握と計画経済<br>価格・生産量・投資を中央が決定]

    B2 -->|予算制約の軟化と情報の欠落| C[観測された結果<br>慢性的な供給不足<br>指標達成の優先と能力の過少報告]
...
この図が分離しているのは、思想そのものと、その実装形態と、実装が生んだ結果の3層です。

同一の構想から、2つの異なる経路が分岐しました。経路1は、生産手段の所有構造を変えず、成果の分配を政策によって調整する方式です。経路2は、所有構造そのものを変更し、配分の決定を中央に集約する方式です。

観測された不具合は、経路2に集中しています。ここで重要なのは、その原因が指導者の資質や意図に帰されていないという点です。コルナイの分析も、第Ⅰ部B-3で参照した分析も、原因を制度の誘因構造に求めています。

すなわち、損失が補填される主体は資源を節約する動機を持たず、指標で評価される主体は指標を最適化し、能力を報告する主体は次期の負担を避けるために過少に報告します。これらはいずれも、各主体にとって合理的な行動です。

第Ⅰ部B-3で扱った情報の問題と、本章で扱った誘因の問題。この2つが重なった点が、経路2の構造的な制約でした。

一方、経路1は現在も多くの国で運用されています。この事実は、構想の内容と実装形態とを区別して評価する必要性を示しています。
