...
日本の平均寿命は、20世紀を通じて延伸しました。この変化は、公衆衛生、医療、栄養、そして労働環境の改善によってもたらされました。第Ⅰ部C-6で記述した上下水道の整備は、その主要な要因の1つです。
...
flowchart TD
    ...
    D --> G
    C3 -->|密度の低下| X[1人あたり維持費用の上昇<br>第Ⅱ部A-4と第Ⅴ部の主題]
...
2.  **数値が示していること**
    ...
    この関係は、第Ⅰ部A-8で参照したターチンらの枠組みにおける、人口と資源の比率という観測項目に対応します。同じ箇所で参照したテインターの分析が示したとおり、問題への対応として追加された仕組みは、それ自体の維持費用を伴います。
...
3.  **本書における位置づけ**
    ...
    制度調整の時期については、第Ⅱ部C-12で整理したとおり、決定の周期を要します。年齢構成の変化は数十年の時間軸で進行するため、原理的には決定が追随できる速度です。ただし調整は給付と負担の再配分を伴うため、第Ⅱ部C-7で整理した決定の困難が作用します。

    第7節に示した死亡の場所の推移は、第Ⅲ部-7で整理する依存の構造に対応します。平時には判断の負荷が軽減され、途絶時にはその判断を担う経験が失われています。

    第Ⅳ部-2で述べる地域社会への期待は、この点と接続します。死を含む生活の判断が専門機関へ移った状態で、途絶時にその判断を誰が担うかという問いです。第Ⅴ部の設計は、この問いに直接は答えません。答えるのは、判断に必要な情報と最小限の供給を、途絶時にも局所で維持する構成についてです。
