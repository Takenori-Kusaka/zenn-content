...
3.  **本書における位置づけ**
    ...
    制度調整の時期については、第Ⅱ部C-10で整理する決定の周期を要します。年齢構成の変化は数十年の時間軸で進行するため、原理的には決定が追随できる速度です。ただし調整は給付と負担の再配分を伴うため、第Ⅱ部C-6で整理する決定の困難が作用します。

    第7節に示した死亡の場所の推移は、第Ⅲ部-5で整理する依存の構造に対応します。平時には判断の負荷が軽減され、途絶時にはその判断を担う経験が失われています。
