...
同じ依存が供給の状態によって利得と負荷に分かれ、負荷を緩和する方法が3つある構造を示します。

```mermaid
flowchart TD
    D[集約型供給への依存<br>分業の帰結]
    D -->|供給が継続する間| A[利得. 注意資源の解放<br>判断を要さない分を<br>他の活動へ配分]
    D -->|供給が途絶した時| B[負荷. 判断の同時多発<br>経験のない判断が<br>逼迫時に要求される]
    subgraph G[事前の備えで緩和]
        direction TB
        K1[帰結1. 平時の機能に<br>埋め込む] ~~~ K2[帰結2. 制度として<br>義務づける]
...
上段の箱が依存です。そこから矢印が2本に分かれます。左は供給が継続する間に実現する利得、右は供給が途絶した時に実現する負荷です。右の負荷の下に枠があり、負荷を緩和する3つの帰結が縦に並びます。

この図が示すのは、依存が利得と負荷の両方を同時に持つという構造です。左右の箱は同じ上段の箱から生じており、一方だけを取り除くことはできません。

したがって、問題の立て方は「依存をなくすべきか」ではありません。「依存の利得を保ちながら、途絶時の負荷をどう緩和するか」です。

下段の枠内に示した3項目は、いずれもこの緩和の方法です。3つに共通するのは、個人の平時の判断に依存しないという点です。枠内の3つに矢印がないのは、いずれか1つを選べばよく、順序の関係がないためです。
