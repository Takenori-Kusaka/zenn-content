---
title: "第Ⅲ部-3　イベントリスクの一覧 ―― 外部入力の定義と、影響の評価軸"
---

### 序論：本章が扱う範囲

本章は、[第Ⅲ部](future-method)のシナリオ構築において入力となるイベントを、一覧として定義します。

各イベントについて、次の4項目を記述します。定義、前章で整理した確度の階層、影響が及ぶ供給基盤、そして観測可能な早期警戒指標です。

本章は、各イベントの発生確率を提示しません。[第Ⅲ部-1](future-method)で述べたとおり、根拠を示せない確率は記載しないためです。提示するのは、発生した場合の影響と、発生の接近を示す指標です。
...
影響：エネルギー供給、食料供給、部材供給。[第Ⅱ部B-2](report-iran-israel-conflict)で示したとおり、代替経路の容量は限定的です。
...
確度：階層3。[第Ⅱ部B-1](report-ukraine-war)で示したとおり、他地域では既に発生しています。
...
確度：階層3。RSA-2048の解読に要する資源は、2021年時点の見積もりでは約2,000万個の雑音のある量子ビットでした。2025年の見積もりは、これを100万個未満へ引き下げています [ [ 325 ] ](https://arxiv.org/abs/2505.15917) 。[第Ⅱ部C-5](quantum-computing-impact)で示した実証規模は105個であり、現在の装置との開きは約4桁です。見積もりの縮小が続いていること自体を、早期警戒指標の1つとして扱います。
...
この図は、[第Ⅰ部A-8](dynastic-cycle-and-corruption)で整理した崩壊の構造を、現在の日本に適用したものです。区分BのB-1からB-6までの6つの事象は、いずれも発生時期を特定できません。
...
したがって、本書が[第Ⅲ部](future-method)で扱う分岐は、次の形を取ります。区分Aによる余力の逓減がどの程度進行した時点で、区分Bのどの事象が発生するか。この組み合わせが、シナリオを分けます。
