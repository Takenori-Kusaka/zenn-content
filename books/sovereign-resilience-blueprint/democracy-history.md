...
flowchart TD
    ...
    C -->|得るもの| G1[利得1. 正統性の周期的な更新<br>決定への同意を定期的に再取得]
    C -->|得るもの| G2[利得2. 誤りの是正経路<br>指導層の交代を暴力なしに実行]
    C -->|支払うもの| P1[費用1. 決定に要する時間<br>討議と合意形成の周期]
    C -->|支払うもの| P2[費用2. 集計の不安定性<br>選択肢が3つ以上のとき順序に依存]
...
この図は、民主主義を制度の良否という軸ではなく、何と何を交換する設計なのかという軸で表現しています。

利得1と利得2は、いずれも第Ⅰ部A-6で扱った崩壊の構造に対応します。第Ⅰ部Aで記述したオスマン帝国と江戸幕府の事例では、正統性の供給が途絶えた段階で崩壊が発生しました。民主的な体制は、正統性を周期的に再取得する手続を制度として備えています。また、指導層の交代を暴力によらず実行する経路を用意しています。

費用1と費用2は、この設計に伴う制約です。費用1は速度の問題であり、費用2は集計の一貫性の問題です。

重要なのは、この交換比率が外部環境によって変動するという点です。外部環境の変化が緩やかであれば、費用1は問題になりません。変化が加速すると、決定周期が環境の変化に追随できなくなります。
