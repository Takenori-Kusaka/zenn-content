...
この一連の要請が、領域を明確に画定し、その内部を均質に管理する統治機構を生みました。第Ⅰ部D-2で記述する火薬と常備軍の関係、すなわち火器の普及が徴税機構の整備を要求したという過程も、この文脈に位置します。
...
```mermaid
flowchart TD
    A[要素1. 境界の画定<br>権限の及ぶ範囲を線として確定<br>1648年以降 段階的に進行]
    A --> B[要素2. 財源の恒常化<br>常備軍の維持に必要<br>課税対象の把握が前提]
    B --> C[要素3. 社会の可読化<br>測量 登録 単位と言語の統一<br>地域固有の慣行を捨象]
    C --> D[結果. 領域内の均質な管理<br>中央が地方を直接把握する構造]
...
この図が示すのは、領域国家の成立が単一の条約によってではなく、3つの要素の連鎖によって進行したという構造です。

要素1が単独で成立しても、統治は変わりません。境界を引いただけでは、その内部を管理する手段がないためです。

要素2が、境界の内部を管理する動機を与えます。常備軍の維持という恒常的な支出が、恒常的な収入を要求します。

要素3が、その収入を得るための手段です。誰がどこに住み、どの土地を耕しているかを把握できなければ、課税は成立しません。

この連鎖の帰結が、中央が地方を直接把握する構造です。第Ⅰ部A-5で記述する明治日本の中央集権化は、この連鎖を約20年で実行した事例にあたります。

なお、この図は近代国家の成立を否定的に描くものではありません。この構造によって、統治の予測可能性が高まり、恣意的な徴発が減少し、広域の公共財を供給できるようになりました。第Ⅰ部B-2で述べる市場の拡大も、この構造を前提とします。
...
3.  **現代社会の現状と課題**
    ...
    第一に、情報の流通は境界に従いません。第二に、第Ⅱ部Bで記述するとおり、供給基盤への攻撃は境界の内側で影響を生じさせます。第三に、気候と感染症は境界を認識しません。

    これらの圧力に対して、領域国家の枠組みがどこまで有効かという問いは、第Ⅲ部および第Ⅳ部の議論に接続します。
