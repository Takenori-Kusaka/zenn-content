...
第Ⅰ部A-8で整理した枠組みに照らすと、この乖離は余力の逓減として作用します。前提1と前提4は外力の側を、前提2と前提3は対応能力の側を変化させます。両者が同時に進行すると、同じ規模の事象に対して、以前は吸収できたものが吸収できなくなります。

### 🗺️ 図：4つの前提と、その現状

4つの前提を、外力の側と対応の側に分けて示します。

```mermaid
flowchart TD
    D[集約型インフラの設計<br>4つの前提]
    subgraph G1[左列：外力の側]
        direction TB
        P1[前提1. 外力は不変<br>現状：極端現象の<br>観測に変化] ~~~ P4[前提4. 標的にならない<br>現状：発電設備への<br>攻撃を観測]
    end
    subgraph G2[右列：対応の側]
        direction TB
        P2[前提2. 需要は維持<br>現状：不均等な<br>人口減少] ~~~ P3[前提3. 財源は継続<br>現状：公債依存 24.2％<br>債務 GDP比 約194％]
    end
    D --> G1
    D --> G2
    G1 --> R[余力の逓減<br>以前は吸収できた事象を<br>吸収できなくなる]
    G2 --> R
    ...
    style R fill:#3f1d1d,stroke:#dc2626,stroke-width:3px,color:#fef2f2
...
この図は、上段の設計から左右2列へ分かれ、下段の1つの帰結へ再び集まる形をしています。左列は外力の側、右列は対応の側です。各列の中に、2つずつの前提が縦に並びます。

左列の前提1と前提4は、外力の側の変化です。設備に加わる負荷が、設計時の想定を超える頻度で生じうる状態です。

右列の前提2と前提3は、対応の側の変化です。負荷に対処するための資源、すなわち利用者と財源が縮小しています。

下段の帰結は、2列が同時に変化することで生じます。4つの前提のうち、いずれか1つが変化しただけであれば、他の前提の余裕によって吸収できます。4つが同時に変化している点が、現在の状況の性質です。
...
1.  **現代社会構造の原点**
    ...
    4つの前提は、第Ⅰ部A-6で記述した明治期の制度設計と、第Ⅰ部Cで記述した技術の経緯とに由来します。人口と経済が拡大し、気候が安定し、国際法が供給設備を保護するという条件のもとで、集約は合理的な選択でした。
