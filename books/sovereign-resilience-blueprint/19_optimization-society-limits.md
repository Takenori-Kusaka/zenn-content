...
この構造は、第Ⅰ部A-8で参照したターチンらの枠組みにおける、人口と資源の比率という観測項目に対応します。
...
長寿化を起点に、3つの延長が1つの作用へ収束する構造を示します。

```mermaid
flowchart TD
    L[長寿化<br>衛生 医療 栄養の成果]
    subgraph G[3つの延長]
        direction TB
        E1[延長1. 就労後の期間<br>貯蓄 年金 家族の支え] ~~~ E2[延長2. 医療と介護<br>個人 保険 公費で分担]
        E2 ~~~ E3[延長3. 世代の重なり<br>扶養と相続の長期化]
    end
    L --> G
    G --> S[制度への作用<br>給付は年齢構成に連動<br>負担は労働力人口に連動]
...
上段の箱が長寿化です。そこから矢印が中段の枠へ入り、枠の中に3つの延長が縦に並びます。枠から出る矢印は1本に集約され、下段の制度への作用へ至ります。

この図の出発点は、長寿化が成果であるという点です。本章は、長寿化を問題として扱いません。扱うのは、成果が生んだ新しい条件です。

中段の3つの延長は、いずれも制度の設計時には想定されていなかった長さに達しています。年金制度、医療保険制度、そして介護保険制度は、それぞれの設計時点における平均余命と年齢構成を前提としていました。

3つの延長が1つの箱へ収束する形は、作用の経路が異なっても帰結が同じであることを示します。給付は年齢構成に連動し、負担は労働力人口に連動します。

第Ⅰ部A-8で整理した枠組みに照らすと、この状況は成功が次の条件を変えるという構造です。テインターの分析が示したとおり、問題への対応として追加された仕組みは、それ自体の維持費用を伴います。

下段に示した作用は、第Ⅲ部の3つのシナリオすべてに共通する前提です。シナリオを分けるのは、この作用に対する制度の調整が、どの時期にどの規模で実施されるかです。
...
1.  **現代社会構造の原点**
    ...
    現在の社会保障制度は、人口の増加が続き、寿命も現在より短かった時期に設計されました。第Ⅰ部A-6で記述した予測可能な貨幣収入という財政原理と同様に、その設計は当時の条件下で合理的でした。
