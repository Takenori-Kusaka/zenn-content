...
flowchart TD
    N[平時<br>系統が機能している状態]
    N -->|仮想発電所として参加| A[系統の需給調整に寄与<br>対価を受け取り費用を回収]
    D[途絶時<br>系統が停止した状態]
    D -->|事前に定めた条件で切り離し| B[局所系統として独立動作<br>近隣の設備と相互に融通]
    B -->|負荷の優先順位に従い| C[優先度の低い負荷から順に制限<br>生存に必要な負荷を最後まで維持]

    style N fill:#1e293b,stroke:#94a3b8,color:#f1f5f9
...
この図は、同じ設備が平時と途絶時とで異なる役割を担うことを示しています。

平時において、設備は系統の一部として機能し、対価を得ます。途絶時において、設備は局所の供給源として機能します。この二重の役割が、第Ⅲ部-7で述べた構成、すなわち途絶時の備えが平時の便益によって支えられる構成の具体形です。

平時から途絶時への移行は、事前に定めた条件によって自動的に生じる必要があります。第Ⅱ部D-3で述べたとおり、途絶時は注意という資源が最も逼迫する局面であり、複雑な操作を要求する設計は機能しません。

Cとして示した負荷の優先順位は、第Ⅳ部-1で述べた「下限の保証」の局所版です。すべての需要を維持するのではなく、生存に必要な需要を最後まで維持する設計です。
